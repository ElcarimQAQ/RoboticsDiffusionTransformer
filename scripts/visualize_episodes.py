...
def main(args):
    ...
    print('hdf5 loaded!!')
    viz_dir = os.path.join(dataset_dir, task_name, 'viz')
    print(f"{viz_dir=}")
    os.makedirs(viz_dir, exist_ok=True)
    visualize_base(base_action, plot_path=os.path.join(viz_dir, dataset_name + '_base_action.png'))
    visualize_joints(qpos, action, plot_path=os.path.join(viz_dir, dataset_name + '_qpos.png'))
    save_videos(image_dict, action, DT,  video_path=os.path.join(viz_dir, dataset_name + '_video.mp4'))
...
def visualize_joints(qpos_list, command_list, plot_path=None, ylim=None, label_overwrite=None):
    ...
    # 计算子图的行数和列数
    num_cols = 2
    num_rows = (num_dim + num_cols - 1) // num_cols
    fig, axs = plt.subplots(num_rows, num_cols, figsize=(12, 2 * num_rows))

    # 确保 axs 是二维数组
    if num_dim == 1:
        axs = axs[np.newaxis, np.newaxis]
    elif num_rows == 1:
        axs = axs[np.newaxis, :]
    ...
    # 隐藏多余的子图
    if num_dim % num_cols != 0:
        for col in range(num_dim % num_cols, num_cols):
            axs[-1, col].axis('off')
